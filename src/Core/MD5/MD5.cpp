...
unsigned int* add(const unsigned char* data, unsigned int len)
{
    ...
    {
        strByte[i >> 2] |= (data[i]) << ((i % 4) * 8);//一个整数存储四个字节，i>>2表示i/4 一个unsigned int对应4个字节，保存4个字符信息
    }
...
string getMD5(const unsigned char* data, int len)
{
    atemp = A;    //初始化
    btemp = B;
    ctemp = C;
    dtemp = D;
    unsigned int* strByte = add(data, len);
